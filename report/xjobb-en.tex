...
As a baseline, a brain which makes random decisions is also used.  
The genetic algorithms provide the genes which are used as input to the decision-making models. 
Statistics such as learning speed, choice of strategy and how well the animals are able to adapt to their surroundings are then analysed. 

The animals are given different tasks in each experiment including food gathering, predator avoidance and mimicking things in the world to survive. 
In each experiment the three decision-making models are compared and contrasted. 
One goal of this project is to find out the advantages and disadvantages of both models, when simulating artificial life.

Genetic algorithms have been used successfully in the past in similar experiments. 
In the Gaia report, the authors acknowledge that many of the behaviours observed could be implemented using simple linear associations \cite{gracias}. 
One of this project's goals is to determine which behaviours that can be observed using the simpler brain types - linear functions and radial-basis functions. 
...
\item Can any evolutionary phenomena be observed in the simulated animals?
\end{enumerate}
...
This section provides a short overview of evolutionary concepts in nature which are mentioned in this report. 
As every solution that the genetic algorithm proposes can be thought of as an individual in a population, many of the normal evolutionary phenomena can be observed in genetic algorithms as well.
...
During extreme environmental changes the adaptation accelerates and once the population can survive the adaptation rate decreases.
...
An example of this is how cheetah and its prey is in a constant battle of being able to run the fastest.

\emph{Mimicry} is a kind of adaptation where one species mimics an object's appearance or behaviour for protection \cite[p.~278]{king}. 
...
Genetic algorithms mimic natural selection in nature by defining a set of genomes, or individuals, where each genome represents a possible solution to the given problem. 
...
In nature, fitness is simply determined by the number of offspring produced by an individual and the percentage of the offspring which survive long enough to generate new offspring\cite[p.~117]{darwin}. 
...
In genetic algorithms, the fitness function can be any value that describes each individual's success at solving the problem at hand. 
...
For this reason, many different fitness functions were examined before settling for the life length of the individuals. 
Using the life length as the fitness function allows the animals to freely choose strategies, as opposed to a function which steers the population toward a certain strategy.

\subsection{Selection}
Selection is the process of selecting which organisms that should be allowed to reproduce and in what proportions . 
In nature, selection is closely tied to fitness, the fittest individual is also selected most often, but in genetic algorithms there are several different ways to model this selection process. 
A good fitness function should strike a balance between favouring the fittest individuals and allowing less fit individuals to survive in a reduced number.  
One trivial selection function could be to simply select all individuals that pass a certain fitness threshold \cite[p.~273]{marsland}. 
This is a bad idea, however, since there are few ways to determine the correct threshold value. 
In the beginning of the evolution, a high threshold will exclude most of the genes due to low general fitness and this will lead to a fast reduction of genetic variation. 
...
A larger fitness value means a larger area on the roulette wheel and the selection function simply generates random values corresponding to the areas of this roulette wheel. 
...
\begin{figure}
\centering\includegraphics[height=0.35\textheight]{roulette.pdf}
...
Mutation is a random, usually small, change of an individual's genome. 
...
When the genome consists of a series of 0:s and 1:s, the mutation operator is simply a flip of that bit and when the genes consist of floating point values, the mutation can be an addition or multiplication of a random value. 
...
\begin{figure}
\centering\includegraphics[scale=0.45]{rbf_1d.png}
...
This makes them useful for function approximation, as any function can be approximated as the sum of a number of weighted radial basis functions. 
Linear functions do not have this property. 
A property of radial basis functions which can both be interpreted as an advantage and disadvantage is that their value never exceeds a given constant, compared to a linear function which can grow to infinitely high or low numbers.
...
% Refer to the image earlier
These libraries were combined to create a model of the world in which the animals were simulated. 
...
The maximum number of red and green plants possible in the world is fixed, and it becomes increasingly probably to spawn a new plant if there only are a few left. 
...
\caption{A screenshot of the simulation, showing green plants (green circles), red plants (slightly larger red circles), herbivores (multicoloured circles with antennae) and a predator (red circle with inner circle and antennae).}
\label{sim_pic}
\end{figure}
...
After that a check for detection occurs and the animals which have detected objects are allowed to process their inputs and apply a $\Delta s$ and a $\Delta r$ (change in speed and rotation) to their current speed and rotation. 
Every animal is then moved in accordance with their speed and rotation. 
This decision process is described in sections \ref{decision_making} to \ref{random_decision_making}.
...
The detection and collision algorithms have a time-complexity of $O(n^{2})$ and therefore scale poorly, which is the reason why the division of the population is needed. 
...
The fourth components are called $l_4$ and $r_4$, and show if an object has been detected or not.
...
This is required to allow the otherwise positive RBF-functions to produce negative values as well. 
...
The difference between using radial basis functions and linear functions is that radial basis functions have a better ability to approximate any decision-making strategy. 
...
Once individuals for the new generation had been selected, crossover and mutation were applied. The probability for applying crossover to a pair of individuals were 30\% and mutation 40\%. Exactly what operators were used is described in the subsequent paragraphs in this section.
...
If a uniform crossover algorithm had been used, it could be the case that the $\mu$ of a radial basis function came from one parent and the $\sigma$ from the other. This would most likely produce an individual with lower fitness than any of the its parents', as each parameter of a certain radial basis function has been tuned to be used with the others. 
Instead, "regions" of genes are replaced when applying crossover. A region is defined as a group of subsequent genes, and for the RBF-based brains the size of these regions are three genes long while one gene long for the linear brains. This means that a radial basis function's $\mu$, $\sigma$ and $A$ are all copied. When crossover is applied, one calculation, i.e. a radial basis function or linear unit, for either speed or rotation and for either the left or right antenna is switched for the parent's. The probability of the first individual's region for the second was 30\% for each region.

A gaussian mutator was chosen as mutation algorithm, as the animals' genes are floating point numbers. When mutation occurred a random number, distributed according to a gaussian distribution with mean $0$ and standard deviation $0.1$, was applied to the gene. On average, one gene was mutated per individual, as recommended in \cite{montana}.
...
The number of green plants is variable and the results are checked after 300 generations. 
...
The expected result of this is a lower average fitness value, but the decision to allow this to happen was based on the comparison of brain types and seeing if they would handle this case differently.
...
\label{exp3_desc}
Predators are added into the world in this experiment.
...
\subsection{Avoiding bad food}
Adding red plants to the simulation caused the fitness of both types of herbivores to decrease. When using a low maximum number of red plants no particular differences between RBF-based and linear brains were found. In both cases they managed to increase their fitness by about 100\% and approximately 40\% of all herbivores died from eating red plants. When doubling the amount of red plants in the simulation differences between the two decision-making models began to appear. Fitness was in both cases increased by about 30\%, but the ratio of herbivores killed from eating red plants differed, as shown in Figure \ref{exp2_deathrates}. This could indicate the use of different strategies. The linear brains seem to be more reckless by weighing the risk of a red plant against the possibility of eating many green plants. In comparison, the RBF-based brains seem to be more focused on avoiding red plants and being more cautious when approaching green plants.This is also supported by the fact that the maximum fitness for the linear brains is much higher but also more unstable compared to the RBF-based brains.
...
Where in section~\ref{exp1_res}, with only herbivores and green plants, they had a fitness increase of about 125\% they were able to increase their fitness by 70-90\% at most for linear brains and 40-50\% at most for RBF-based brains. 
...
The locality property means that a small change in colour can still give the same, or similar, reaction, which is believed to make them better predators as the herbivores' colours are dynamic during the initial phase of the simulation.
...
Difficulty when selecting which individuals are allowed to pass on their genes can give a decrease of the effectiveness of genetic algorithms. 
...
An explanation as to why this did not occur could be that the motivation to increase one colour's component while decreasing the two others is not apparent until perfect mimicry already has been reached.
